...
// Local Runtime Types
use gm_chain_runtime::{opaque::Block, AccountId, Balance, Hash, Index as Nonce, RuntimeApi};
...
impl ChainIdentify for Box<dyn ChainSpec> {
    fn is_solo_dev(&self) -> bool {
        self.id().starts_with("gm-solo-dev")
    }
}
...
impl sc_executor::NativeExecutionDispatch for TemplateRuntimeExecutor {
    ...
    fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
        gm_chain_runtime::api::dispatch(method, data)
    }

    fn native_version() -> sc_executor::NativeVersion {
        gm_chain_runtime::native_version()
    }
}
...
pub fn start_solo_dev<RuntimeApi, Executor>(
    config: Configuration,
    ...
    Executor: sc_executor::NativeExecutionDispatch + 'static,
{
    ...
    } = new_solo_partial::<RuntimeApi, Executor>(&config)?;
...
#[allow(clippy::type_complexity)]
pub fn new_solo_partial<RuntimeApi, Executor>(
    config: &Configuration,
) -> Result<
    PartialComponents<
        TFullClient<Block, RuntimeApi, NativeElseWasmExecutor<Executor>>,
