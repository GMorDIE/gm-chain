...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + Sized {
        ...
        /// Currency type for aye votes.
        type AyeCurrency: ReservableCurrency<Self::AccountId>
            + LockableCurrency<Self::AccountId, Moment = Self::BlockNumber, Balance = Self::Balance>;

        /// Currency type for nay votes.
        type NayCurrency: ReservableCurrency<Self::AccountId>
            + LockableCurrency<Self::AccountId, Moment = Self::BlockNumber, Balance = Self::Balance>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn vote(
            ...
            //          AccountVote::<AyeBalanceOf<T>>::Standard { vote, balance }
            //      }
            //     AccountVote::Split { aye, nay } => AccountVote::<AyeBalanceOf<T>>::Split {
            //         aye,
            //         nay: nay.into(),
            //     },
            // };
...
impl<T: Config> Pallet<T> {
    ...
    fn try_vote(
        ...
        let (aye, nay) = vote.balance_split();

        ensure!(
            if let Some(v) = aye {
                v <= T::AyeCurrency::free_balance(who)
            } else {
                true
            } && if let Some(v) = nay {
                v <= T::NayCurrency::free_balance(who)
            } else {
                true
            },
        ...
        // votes are in place.
        if let Some(v) = aye {
            T::AyeCurrency::extend_lock(DEMOCRACY_ID, who, v, WithdrawReasons::TRANSFER);
        }
        if let Some(v) = nay {
            T::NayCurrency::extend_lock(DEMOCRACY_ID, who, v, WithdrawReasons::TRANSFER);
        }
