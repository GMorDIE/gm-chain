...
fn currency_adapter_ensure_currency_adapter_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert!(
                <Runtime as pallet_elections_phragmen::Config>::Currency::can_slash(
                    &TREASURY_ACCOUNT,
                    10
                )
            );
            ...
            assert!(
                <Runtime as pallet_elections_phragmen::Config>::Currency::can_reserve(
                    &TREASURY_ACCOUNT,
                    5
                )
            );
            ...
            assert_ok!(
                <Runtime as pallet_elections_phragmen::Config>::Currency::transfer(
                    &TREASURY_ACCOUNT,
                    &ALICE,
                    11,
...
fn currency_adapter_burn_must_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                TreasuryCurrencyAdapter::total_issuance(),
                init_total_issuance - 10
            );
            drop(imbalance);
            assert_eq!(
                TreasuryCurrencyAdapter::total_issuance(),
...
fn currency_adapter_reserving_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            TreasuryCurrencyAdapter::total_balance(&TREASURY_ACCOUNT),
            111
        );
        assert_eq!(
        ...
        assert_eq!(
            TreasuryCurrencyAdapter::reserved_balance(&TREASURY_ACCOUNT),
            0
        );
...
fn currency_adapter_slashing_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        let _ = TreasuryCurrencyAdapter::deposit_creating(&TREASURY_ACCOUNT, 111);
        assert_ok!(TreasuryCurrencyAdapter::reserve(&TREASURY_ACCOUNT, 69));
        assert!(TreasuryCurrencyAdapter::slash(&TREASURY_ACCOUNT, 69)
            .1
            .is_zero());
...
fn currency_adapter_slashing_reserved_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            TreasuryCurrencyAdapter::slash_reserved(&TREASURY_ACCOUNT, 42).1,
            0
        );
        assert_eq!(
            TreasuryCurrencyAdapter::reserved_balance(&TREASURY_ACCOUNT),
...
fn currency_adapter_slashing_incomplete_reserved_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            TreasuryCurrencyAdapter::slash_reserved(&TREASURY_ACCOUNT, 69).1,
            27
        );
...
fn currency_adapter_slashing_named_reserved_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        let _ = TreasuryCurrencyAdapter::deposit_creating(&TREASURY_ACCOUNT, 111);
        assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 111);
        assert_ok!(TreasuryCurrencyAdapter::reserve_named(
            &RID_1,
            &TREASURY_ACCOUNT,
            111
        ));
...
fn currency_adapter_repatriating_named_reserved_balance_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(TreasuryCurrencyAdapter::free_balance(&TREASURY_ACCOUNT), 0);
        assert_eq!(
            TreasuryCurrencyAdapter::reserved_balance_named(&RID_1, &ALICE),
            0
        );
        assert_eq!(TreasuryCurrencyAdapter::free_balance(&ALICE), 43);
    });
